...
class DeepseekAttention(nn.Module):
    ...
    def forward(self, hidden_states_q: torch.Tensor, hidden_states_kv: torch.Tensor, q_position_ids: torch.LongTensor, 
        ...
        #query_states : torch.Size([1, 128, 1, 192])
        #key_states : torch.Size([1, 128, 20480, 192])
        #value_states : torch.Size([1, 128, 20480, 128])
        query_states=query_states.transpose(1,2)
        key_states=key_states.transpose(1,2)
        value_states=value_states.transpose(1,2)
        # #print(query_states.shape)
        
        attn_output = memory_efficient_attention_forward(query_states,key_states,value_states)
